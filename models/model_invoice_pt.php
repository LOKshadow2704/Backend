...
class model_invoice_pt
{
    ...
    public function get_invoiceByPT($id)
    {
        $connect = $this->db->connect_db();
        if ($connect) {
            $query = "SELECT h.IDHoaDon, h.IDKhachHang, h.TrangThaiThanhToan, h.NgayDangKy, h.NgayHetHan,
                            h.TrangThai, tk.HoTen , tk.SDT 
                        FROM 
                            hoadonthuept h
                        JOIN 
                            khachhang kh ON h.IDKhachHang = kh.IDKhachHang
                        JOIN 
                            taikhoan tk ON kh.TenDangNhap = tk.TenDangNhap
                        WHERE 
                            h.IDHLV = ?";
            $stmt = $connect->prepare($query);
            $stmt->execute([$id]);
